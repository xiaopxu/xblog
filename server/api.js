...
router.post('/api/signin', async(req, res) => {
    ...
    try {
        ...
        if (user.password === password) {
            let oldRemember = await dao
                .loginRememberDao
                .findRememberByUserId(user._id)
            if (oldRemember !== null) {
                await dao
                    .loginRememberDao
                    .delRememberByUserId(user._id)
            }
            //插入新的rememberKey
            let newData = {
                userId: user._id,
...
router.post('/api/autoSignin', async(req, res) => {
    ...
    try {
        ...
        let remember = await dao
                .loginRememberDao
                .findRememberByKey(rememberKey)
